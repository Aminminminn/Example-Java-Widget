...
#include "t1decode.h" /* for t1 seac    */
   ...
   * [MicroEJ]
   *
   * The macro MEJ_FT_ALLOC maps the Freetype macro FT_ALLOC. This macro is
   * designed to be only used in the function cf2_interpT2CharString(). This
   * function declares a field "FT_Error* error" whereas the FT allocation
   * function requires a field "FT_Error error". On error, this function
   * go to the label "exit".
   */
#define MEJ_FT_ALLOC(ptr, size) { 					\
		FT_Error error; 							\
		if (FT_ALLOC(ptr, size)) {					\
...
#define PS_STORAGE_SIZE 3
    ...
    CF2_GlyphPath  glyphPath = NULL;

    CF2_HintMap counterHintMap = NULL;
    CF2_HintMask counterMask = NULL;
    ...
    // [MicroEJ] use FT heap to save 11776 bytes on the stack
    MEJ_FT_ALLOC(glyphPath, sizeof(*glyphPath));
    ...
    while ( 1 )
    {
      ...
      case cf2_cmdHINTMASK:
        ...
        else
        {
          ...
          // [MicroEJ] use FT heap to save 3692 bytes on the stack
          MEJ_FT_ALLOC(counterHintMap, sizeof(*counterHintMap));
          MEJ_FT_ALLOC(counterMask, sizeof(*counterMask));
