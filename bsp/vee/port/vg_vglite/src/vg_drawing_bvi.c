...
static DRAWING_Status _draw_string_on_circle(MICROUI_GraphicsContext *gc, const jchar *text, jint faceHandle,
	...
	if (UI_VGLITE_enable_vg_lite_scissor(gc)) {
		...
		jint error = VG_FREETYPE_draw_string(&_add_glyph_color, text, faceHandle, size, matrix, vglite_color,
...
static DRAWING_Status _draw_string_on_circle_gradient(MICROUI_GraphicsContext *gc, const jchar *text, jint faceHandle,
	...
	if (UI_VGLITE_enable_vg_lite_scissor(gc)) {
		...
		if (VG_LITE_SUCCESS == vglite_error) {
			...
			// add operation
			target = MAP_BVI_ON_GC(gc);
			jint error = VG_FREETYPE_draw_string(&_add_glyph_gradient, text, faceHandle, size, matrix, 0,
			                                     letterSpacing, radius, direction, &data);
			VG_VGLITE_HELPER_handle_error(gc, error);
...
DRAWING_Status VG_DRAWING_BVI_drawString(MICROUI_GraphicsContext *gc, const jchar *text, jint faceHandle, jfloat size,
	...
	return _draw_string_on_circle(gc, text, faceHandle, size, matrix, alpha, blend, letterSpacing, (jfloat)0, (jint)0);
}
...
DRAWING_Status VG_DRAWING_BVI_drawStringGradient(MICROUI_GraphicsContext *gc, const jchar *text, jint faceHandle,
                                                 ...
                                                 const jfloat *gradientMatrix) {
	return _draw_string_on_circle_gradient(gc, text, faceHandle, size, matrix, alpha, blend, letterSpacing, (jfloat)0,
	                                       (jint)0, gradientData, gradientMatrix);
}
