...
int32_t LLSEC_PUBLIC_KEY_IMPL_get_encoded_max_size(int32_t native_id) {
	...
	if (LLSEC_MBEDTLS_SUCCESS != mbedtls_rc) {
		 ...
		 * mbedtls_pk_write_pubkey_der() API will write data at the end of the buffer, not at the beginning, so instead
		 * of getting the encoded max size (mbedTLS v3.x has macros, mbedTLS v2.x doesn't), get the encoded fixed size.
