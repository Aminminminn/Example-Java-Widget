...
int32_t     LLSEC_SECRET_KEY_FACTORY_PBKDF2_mbedtls_get_key_data(LLSEC_secret_key *secret_key,
...
int32_t LLSEC_SECRET_KEY_FACTORY_PBKDF2_mbedtls_get_key_data(LLSEC_secret_key *secret_key, mbedtls_md_type_t md_type,
                                                             uint8_t *password, int32_t password_length, uint8_t *salt,
                                                             int32_t salt_length, int32_t iterations,
                                                             int32_t key_length) {
	...
	if (LLSEC_SUCCESS == return_code) {
		return_code = (int32_t) secret_key;
		               LLSEC_SECRET_KEY_FACTORY_DEBUG_TRACE(
						   "%s mbedtls_pkcs5_pbkdf2_hmac() success. (native_id = %d)\n", __func__,
						   (int)return_code);
	} else {
 ...
 * @param[in] password        the password to encode, a null terminated '\0' byte array representation of the format in
 * String
...
int32_t LLSEC_SECRET_KEY_FACTORY_IMPL_get_key_data(int32_t algorithm_id, uint8_t *password, int32_t password_length,
	...
	if (NULL == secret_key) {
		...
		return_code = algorithm->get_key_data(secret_key, algorithm->md_type, password, password_length, salt,
