...
#ifdef CMSIS_CYCLES
#define MYTIMEDIFF(fin,ini) ((CORE_TICKS)(((CORETIMETYPE)(fin)-(ini)) / TIMER_RES_DIVIDER))
#define CLOCKS_PER_SEC (configCPU_CLOCK_HZ)
#define TIMER_RES_DIVIDER (5)
#else
...
static CORETIMETYPE start_time_val, stop_time_val = 0;
/* Function : start_time
...
{
	/* Here we need a precise timing, this is why we use CMSIS cycles */
	uint32_t cycles = KIN1_GetCycleCounter();
	return (uint64_t)(cycles / (configCPU_CLOCK_HZ / 1000000));
}
