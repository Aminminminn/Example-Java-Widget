 ...
 * @return: -1 if there is not enough memory in the buffer, otherwise the new path_offset
 */
static int __cubic_to(int16_t cx1, int16_t cy1, int16_t cx2, int16_t cy2, int16_t x, int16_t y);
 ...
 * - otherwise: the updated path offset
 */
static int __update_path(vg_lite_path_t *path, bool first_path, bool last_path, int left, int top, int right,
                         int bottom);
 ...
 * - otherwise: the updated path offset
 */
static int __approximate_ellipse_arc_to(int radius_w, int radius_h, float32_t start_angle, float32_t end_angle);
 ...
 * - otherwise: the updated path offset
 */
static int __approximate_ellipse_arc_fragment_to(int radius_w, int radius_h, float32_t start_angle,
                                                 float32_t end_angle);
 ...
 * - otherwise: the updated path offset
 */
static int __approximate_ellipse_arc_fragment(uint16_t center_x, uint16_t center_y, int radius_w, int radius_h,
                                              float32_t start_angle_rad, float32_t end_angle_rad, float32_t tangent,
                                              int revert);
 ...
 * - otherwise: the updated path offset
 */
static int __multiple_quarter_curve_to(int radius_w, int radius_h, int start, int number);
 ...
 * @return: -1 if there is not enough memory in the buffer, otherwise the new path_offset
 */
static int __approximate_ellipse_arc(int radius_out_w, int radius_out_h, int radius_in_w, int radius_in_h,
                                     float32_t start_angle_rad, float32_t arc_angle_rad, int caps);
...
int UI_DRAWING_VGLITE_PATH_compute_rectangle(
	...
	return __update_path(rectangle_shape, first_path, end_path, left, top, right, bottom);
}
...
int UI_DRAWING_VGLITE_PATH_compute_rounded_rectangle(
	...
	return __update_path(rounded_rectangle_shape, first_path, end_path, left, top, right, bottom);
}
...
int UI_DRAWING_VGLITE_PATH_compute_ellipse_arc(
	...
	if (fill == false) {
		...
		(void)__line_to(
			(int)(__ctxt.center_x + (__ctxt.sin * radius_in_w)), (int)(__ctxt.center_y - (__ctxt.cos * radius_in_h)));
	...
	return __update_path(ellipse_arc, true, true, -radius_out_w, -radius_out_h, radius_out_w, radius_out_h);
}
...
int UI_DRAWING_VGLITE_PATH_compute_ellipse(
	...
	return __update_path(ellipse_shape, first_path, end_path, -radius_w, -radius_h, radius_w, radius_h);
}
...
int UI_DRAWING_VGLITE_PATH_compute_filled_ellipse(
	...
	return __update_path(point_shape, true, false, -CIRCLE_RADIUS, -CIRCLE_RADIUS, +CIRCLE_RADIUS, +CIRCLE_RADIUS);
}
...
int UI_DRAWING_VGLITE_PATH_compute_thick_shape_line(
	...
	cap = MEJ_VGLITE_PATH_GET_CAPS_END((unsigned int)caps);
	switch (cap) {
	...
	cap = MEJ_VGLITE_PATH_GET_CAPS_START((unsigned int)caps);
	switch (cap) {
	case DRAWING_ENDOFLINE_ROUNDED:
	{
		// Cubics for end cap
	...
	return __update_path(thick_line_shape, true, true, -half_thickness, top, length + half_thickness, bottom);
}
...
int UI_DRAWING_VGLITE_PATH_compute_thick_shape_ellipse_arc(
	...
	if (l_arc_angle_deg < 0) {
		...
		int cap1 = MEJ_VGLITE_PATH_GET_CAPS_START((unsigned int)l_caps);
		int cap2 = MEJ_VGLITE_PATH_GET_CAPS_END((unsigned int)l_caps);
		l_caps = 0;
		l_caps |= MEJ_VGLITE_PATH_SET_CAPS_START((unsigned int)cap2);
		l_caps |= MEJ_VGLITE_PATH_SET_CAPS_END((unsigned int)cap1);
	}
	...
	return __update_path(thick_ellipse_arc_shape, true, true, -radius_out_w, -radius_out_h, +radius_out_w,
	                     +radius_out_h);
}
...
static int __approximate_ellipse_arc(
	...
	// Compute end cap
	int cap = MEJ_VGLITE_PATH_GET_CAPS_END((unsigned int)caps);
	switch (cap) {
	case DRAWING_ENDOFLINE_ROUNDED:
	{
		// Compute rounded cap
		...
		(void)__approximate_ellipse_arc_fragment(
			cap_x, cap_y,
			(uint16_t)cap_radius, (uint16_t)cap_radius, cap_start_angle, cap_end_angle, CAP_TAN, 0);
		break;
	}
...
static int __approximate_ellipse_arc_fragment(
	...
	int16_t Ay = (int16_t)(radius_h * -arm_sin_f32(start_angle_rad));  // minus because y axis is inverted (0,0 is top
	                                                                   // left)
